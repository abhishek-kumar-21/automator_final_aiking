...
const fetchBlogsFromAPI = async (skill: string): Promise<Blog[]> => {
  try {
    ...
    const mappedTag =
      SKILL_TO_DEVTO_TAG[skill] ?? normalizedSkill.replace(/[^a-z0-9]/gi, "");
...
const SkillBlogs = () => {
  ...
  if (loading)
    return (
      <p className="text-center text-slate-500 py-8 font-inter">
        ...
        >
          <CardHeader className="flex flex-col gap-2 p-0 mb-4">
            <div className="flex items-center gap-3">
              ...
              <CardTitle className="text-lg font-raleway font-bold text-slate-900">
              ...
              {blogs[skill]?.length > 0 ? (
                blogs[skill].map((blog, idx) => (
                  <a
                    ...
                    className="group rounded-lg bg-slate-50 border border-slate-200 hover:border-blue-500 hover:bg-blue-50/30 transition-all duration-300 overflow-hidden"
                  >
                    ...
                    <div className="p-3 flex justify-between items-start gap-2">
                      <p className="text-sm text-slate-800 font-raleway font-semibold leading-tight line-clamp-2 group-hover:text-blue-700 transition-colors">
                        {blog.title}
                      </p>
